...
def PI_reward(molecule):
  ...
  try:
    ...
    pass
  try:
    modelgap = load('10k_gap_model.joblib')
    gapreward = modelgap.predict(X.reshape(1, -1))
    print(f'predicted homo-lumo gap = {gapreward[0][0] * -1}')
  except TypeError:
    print(f'TypeError on mol: {Chem.MolToSmiles(mH)} -- gap prediction')
    gapreward = [[0]]
  PIreward = (co2reward[0][0] * -1) - (gapreward[0][0])
  return PIreward 
