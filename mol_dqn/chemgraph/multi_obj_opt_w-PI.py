...
class MultiObjectiveRewardMolecule(molecules_mdp.Molecule):
  ...
  def PI_reward(smiles):
    ...
    obConversion.WriteFile(mol, './database/'+n+'.xyz')
    obConversion.ReadFile(mol, "1ABC.pdb.gz")
    X=VariancePersist('./database/'+n+'.xyz'.format(i+1), pixelx=pixelsx, pixely=pixelsy, myspread=spread, myspecs={"maxBD": Max, "minBD":-.10}, showplot=False)
    model = load('saved_PI_CO.joblib')
